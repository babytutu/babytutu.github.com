<template>
  <div class="audio-player-container" :class="{ mini }">
    <div class="audio-player-warper">
      <OBJECT :id="id" height="0" width="0" CLASSID="CLSID:6BF52A52-394A-11d3-B153-00C04F79FAA6">
        <param name="autoStart" value="false" />
      </OBJECT>
      <div class="audio-player-control-btn">
        <template v-if="!error">
          <svg viewBox="-9 0 32 32" class="audio-player-control-icon" v-if="!playing" @click="play">
          ...
          <svg viewBox="0 0 17 32" class="audio-player-control-icon" v-else @click="pause">
      ...
      <div class="audio-player-warper-content" v-if="!mini">
        ...
        <div class="audio-player-error" v-if="error">文件加载失败</div>
        <div class="audio-player-control" v-else>
          ...
          <svg viewBox="0 0 32 32" class="audio-player-control-icon volume" v-if="muted" @click="toggleMute">
...
/**
 * @see https://docs.microsoft.com/en-us/windows/win32/wmp/object-model-reference-for-scripting
 */
export default {
  name: 'player-ie',
  props: {
  ...
  data () {
    return {
      playBackRate: 1, // 默认倍速
      id: `player${(Math.random(1) * 1000).toFixed(0)}`,
      mini: false,
      muted: false,
      volume: 1,
      rate: 0,
  ...
  beforeUnmount () {
    // console.log('beforeUnmount触发停止播放')
    this.closePlayer()
  },
  methods: {
    /**
    ...
    play () {
      ...
      this.player.url = this.value.src
      this.player.settings.volume = this.volume * 100 // 初始化音量
      this.player.settings.rate = this.playBackRate // 初始化倍速
      this.timer = window.setInterval(() => {
        this.duration = this.player && (this.player.currentMedia.durationString || '--:--')
        this.currentTime = this.player && (this.player.controls.currentPositionString || '00:00')
        this.rate = this.player && (this.player.controls.currentPosition / this.player.currentMedia.duration)
      }, 1000)
    ...
    pause () {
      this.playing = false
      this.player && this.player.controls.pause()
    ...
    changeTime (rate) {
      if (this.player) {
        this.player.controls.currentPosition = this.player.currentMedia.duration * rate
      }
    },
    /**
     * 调整倍速
    ...
    changeRate (rate) {
      ...
      this.playBackRate = rate // 记录变更的倍速
    },
    /**
     * 调整音量
     */
